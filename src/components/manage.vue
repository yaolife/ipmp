    ...
    async getMenuManagerGetFormMenuTree() {
        ...
        if (item.children && item.children.length > 0) {
          item.children.forEach((items, indexs) => {
            newMenu[index].children.push({
              ...
              url: items.url,
              iconNor: items.iconNor,
              badge: items.badge,
            });
            // 三级菜单
            if (items.children && items.children.length > 0) {
              items.children.forEach((_item, _index) => {
                newMenu[index].children[indexs].children.push({
                  children: [],
                  name: _item.name,
                  resource: _item.resource,
                  url: _item.url,
                  iconNor: _item.iconNor,
                });
              });
            }
          });
      ...
      try {
        ...
        if (totalMenus && totalMenus.length !== 0) {
          ...
          let menus = [];
          // 一级菜单
          let url = "/data_management_modules/";
          let customUrl = "/custom_page_manage_modules/";
          list.forEach((item, index) => {
            ...
            if (item.menuType === "custom") {
              ...
              itemUrl =
                url + item.id + "/" + item.menuName + "/" + item.menuCode;
            }
            ...
            if (item.children.length > 0) {
              item.children.forEach((items, indexs) => {
                ...
                if (!items.isDetail) {
                  menus[index - 1].children.push({
                    children: [],
                    icon: "none",
                    url: itemsUrl,
                    name: items.menuName,
                    resource: items.menuCode,
                  });
                }
                ...
                if (items.children.length > 0) {
                  items.children.forEach((_item, _index) => {
                    ...
                    menus[index - 1].children[indexs].children.push({
                      children: [],
                      icon: "none",
                      url: _itemsUrl,
                      name: _item.menuName,
                      resource: _item.menuCode,
                    });
                  });
                }
              });
