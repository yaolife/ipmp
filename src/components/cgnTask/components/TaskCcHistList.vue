<template>
  <div class="task-tab-content">
    <!-- 查询条件 -->
    <!-- <new-query
      class="cud-office-search-block-ch"
      ref="queryDialog"
      :fields="queryFields"
      :moreMenus="moreMenus"
    ...
    <div ref="searchRef">
      <query-form
        :queryFormId="'offce_task_cc_hist_list'"
      ...
      <div class="cud-card-list">
        <div v-for="(item, index) in data" :key="item.id" class="cud-card-item"
          ...
          <div class="desc">
            ...
            <!-- <p>{{ $t('cgnTask.readList.assignTime') }}：{{ item.assignTime }}</p> -->
            <p>{{ $t("cgnTask.readList.assignTime") }}：{{ item | fieldValue("assignTime", $i18n) }}</p>
          </div>
          <div class="button">
            <el-button style="width: 100%;" @click="$emit('openTaskForm', item)">查看</el-button>
          </div>
        </div>
      </div>
 ...
 */
export default {
  name: "CgnTaskCenterTaskCcHistList",
  components: {
    // NewQuery,
    queryForm,
  },
  ...
  methods: {
    ...
    getFieldName(fieldKey) {
      return "cgnTask.readList." + fieldKey;
    },
    resetSeniorSearch() {
