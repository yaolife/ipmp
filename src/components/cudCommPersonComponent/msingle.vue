      ...
      let timer = setTimeout(function () {
        ...
        for (var i = 0; i < that.myData.length; i++) {
          for (var j = 0; j < value.length; j++) {
            if (that.myData[i].empId == value[j]) {
              arr1.push(that.myData[i]);
            }
          }
        }
        ...
        arr1.map((item) => {
          arr.map((item1) => {
            if (item.empId == item1) {
              arr3.push(item.empName + "-" + item.workDeptNamePath + "\n");
            }
          });
        });
        
        that.workDeptName = arr3.join("");
        that.$set(that, "workDeptName", that.workDeptName);
        clearTimeout(timer);
      }, 200);
    },
