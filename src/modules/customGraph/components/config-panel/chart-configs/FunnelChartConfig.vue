...
export default {
  ...
  methods: {
    ...
    handleDialogConfirm() {
      try {
        // 检查是否有输入值
        if (!this.tempStaticDataStr || this.tempStaticDataStr.trim() === "") {
          this.$message.error("请输入数据内容");
          return;
        }
        ...
        if (!parsedData.seriesData || !Array.isArray(parsedData.seriesData)) {
          throw new Error("数据格式错误，必须包含 seriesData 数组");
        }
        // 更新配置中的静态数据
        ...
        this.$message.error(error.message || "JSON格式错误，请检查输入");
      }
    },
    ...
    handleSave() {
      ...
      if (
        ...
        (!this.localConfig.dataUrl || this.localConfig.dataUrl.trim() === "")
      ) {
        this.$message.error("请输入动态数据源URL");
        return;
      }
      ...
      if (
        ...
        (!this.localConfig.tempStaticDataStr ||
          !this.localConfig.tempStaticDataStr.seriesData)
      ) {
        this.$message.error("请先编辑静态数据源内容");
        return;
      }
      ...
      this.$confirm("确认重置所有配置？", "提示", {
        ...
        .then(() => {
          ...
          this.localConfig = {
            ...
            dataSourceType: "dynamic",
            dataUrl: "",
            tempStaticDataStr: JSON.parse(
              JSON.stringify(this.$data.localConfig.tempStaticDataStr)
            ),
          };
