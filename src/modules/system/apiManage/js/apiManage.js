...
export default {
  ...
  methods: {
    fetchData() {
      ...
      api.getFaceInfoListAPI(fullQueryParams)
        .then(result => {
    ...
    setCronParams(){
      ...
      if(!!this.editForm.checkFrequency){
        this.editForm.frequencyCron = "0 */" + this.editForm.checkFrequency + " * * * ?";
      }
    },
    ...
    onClickDelete(row) {
      this.$confirm(
        ...
        api.deleteFaceInfoAPI(row).then(response => {
    ...
    onClickAdd() {
      ...
      this.$refs.editForm.validate((valid) => {
        ...
        if (valid) {
          ...
          api.saveFaceInfoAPI(param).then(response => {
    ...
    onClickModify() {
      ...
      this.$refs.editForm.validate((valid) => {
        ...
        if (valid) {
          let param = {...this.editForm};
          param.isEnable = param.isEnable ? "0" : "1";
          api.modifyFaceInfoAPI(param).then(response => {
            if (response.data.code === '0') {
