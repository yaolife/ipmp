...
// 用户基本信息 /dp/api/user-data-permission/userInfo
export const getUserInfo = (userId) => {
  return request.post(`/dp/api/user-data-permission/userInfo/${userId}`);
};
...
// 用户对实体的列权限信息 /dp/api/user-data-permission/col-data-perm/list
export const getUserColDataPermList = async (data) => {
  const r = await request.post(`/dp/api/user-data-permission/col-data-perm/list`,data)
  if (r.data.code == 0 && r.data.data && r.data.data.fieldColDataPermissions) {
    return r.data.data.fieldColDataPermissions
  }
  return []
}

// 用户对实体的行权限信息 /dp/api/user-data-permission/row-data-perm/list
export const getUserRowDataPermList = async (data) => {
  const r = await request.post(`/dp/api/user-data-permission/row-data-perm/list`,data) 
...
export const rolePermissionApi = {
  ...
  /**
   * 获取字段权限配置列表
   * @description 获取指定资产的字段权限配置
   * @param {number} assetId - 资产ID
   * @returns {Promise<Array>} 返回字段权限配置列表
   */
  async getFieldPermissionsList(assetId) {
    await mockDelay(200)
