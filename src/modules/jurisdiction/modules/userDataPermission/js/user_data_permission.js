import {
  ...
  getUserInfo,
  getPermedEntities,
  getUserColDataPermList,
  getUserRowDataPermList
} from "../api/index.js";
...
export default {
  ...
  methods: {
    ...
    async loadRoleInfo() {
      ...
      try {
        let res = await getUserInfo(this.jobNumber || "");
        if (res.data.code != 0) {
          return this.$message.error(res.data.message || "加载角色信息失败");
        }
        if (
          res.data.data &&
          res.data.data.roles &&
        ...
        this.$message.error(`${error.message || "加载角色信息失败"}`);
      } finally {
    ...
    async loadAssetList() {
      ...
      try {
        ...
        this.assetList = entities.map(entity => ({
          id: entity.entityId,
    ...
    // 选择实体
    handleAssetSelect(asset, isReload) {
      // 防止重复选择同一个实体
      if (
        this.selectedAsset &&
        this.selectedAsset.code === asset.code &&
        !isReload
      ) {
        return;
      }
      ...
      this.loadPermissions(asset.code);
      this.tempPermissionSelections = [];
    },

    // 加载权限数据
    async loadPermissions(assetCode) {
      const userId = this.roleInfo.userId;
      if (!userId) {
      ...
      try {
        ...
        const [
          ...
          getUserColDataPermList({ userId: userId, entityCode }),
        ...
        this.$nextTick(() => {
          this.rowPermissions = rowPermissions.map(rowPermission => {
            return {
              ...rowPermission,
              displayName: this.generateRowPermissionDisplayName(
                rowPermission.rowDataPermDetailList
              )
            };
          });
    ...
    async getOptions(params) {
      try {
        ...
        if (comData[0] && comData[0].dataSource == "URL") {
          ...
          let params;
          if (
            comConfig.requestType.toLocaleLowerCase() === "get" &&
            comConfig.requestParams
          ) {
            params = new URLSearchParams(
              JSON.parse(comConfig.requestParams) || {}
            );
          }
          const data = await request[comConfig.requestType.toLocaleLowerCase()](
            comConfig.requestType.toLocaleLowerCase() === "post"
              ? comConfig.dataUrl
              : `${comConfig.dataUrl}?${params}`,
            comConfig.requestType.toLocaleLowerCase() === "post"
              ? JSON.parse(comConfig.requestParams || "{}")
              : null
          );
          console.log("自定义接口data", data);
          if (data.data.code != 0) {
            return [];
          }
          ...
          const res = await rolePermissionApi.getDictTreeItem({
            dictCode: dictCode,
            itemStatus: "1",
            itemText: ""
          });
          return (
            res.data.data.row.map(item => {
              return {
                label: item.itemText,
    ...
    async handleSaveRowPermission() {
      try {
        const roleCode = this.$route.params.roleCode || this.jobNumber || "";
        ...
        this.tempPermissionSelections.forEach(selection => {
          if (selection.selectedMethod === "all") {
            ...
            // 指定范围权限
            const values = selection.config.values
              .map(item => `${item}`)
              .join(",");
            rowDataPermDetailList.push({
              entityCode: entityCode,
            ...
            selection.selectedMethod === "custom" ||
            (selection.selectedMethod === "customize_sql_condition" &&
              selection.config)
          ) {
            // 自定义条件权限
            rowDataPermDetailList.push({
        ...
        if (this.isEditingRowPermission) {
          ...
          try {
            ...
            // 只有更新成功后才执行后续操作
            await this.loadPermissions(this.selectedAsset.code);
      ...
      this.$confirm("确认删除这条行权限吗?", "提示", {
        ...
        .then(async () => {
          try {
            ...
            await rolePermissionApi.deleteRowDataPermission(
              permission.rowDataPermissionId
            );
      ...
      const descriptions = item.rowDataPermDetailList
        .map(detail => {
          ...
          if (method === "all") {
            ...
            method === "custom" ||
            method === "customize_sql_condition"
          ) {
            ...
            switch (detail.rowPermControlMethod) {
              ...
              case "appoint":
                if (detail.dataScopeValue) {
                  const valueString = detail.dataScopeValue
                    .replace(/[()]/g, "")
                    .replaceAll(/'/g, "");
                  const values = valueString
                    ...
                    .map(v => v.trim())
                    .filter(v => v);
                  const handled = this.allOptions[detail.fieldCode]
              ...
              case "customize_sql_condition":
                if (detail.dataScopeOperator && detail.dataScopeValue) {
                  ...
                  // 根据字段类型获取操作符映射
                  const availableOperators = this.getOperatorsByFieldType(
                    detail.fieldCode
                  );
                  const selectedOperator = availableOperators.find(
                    op => op.code === detail.dataScopeOperator
                  );
                  const operatorText = selectedOperator
                    ? selectedOperator.name
                    : detail.dataScopeOperator;
    ...
    initDefaultTempPermissionSelections() {
      if (this.entityPermPoints && this.entityPermPoints.rowPermPointList) {
        this.tempPermissionSelections = this.entityPermPoints.rowPermPointList.map(
          (point, index) => {
    ...
    generateColumnPermissionDisplayName(code) {
      const d =
        (this.entityPermPoints &&
          this.entityPermPoints.colPermPointList &&
          this.entityPermPoints.colPermPointList.find(i => {
    ...
    generateRowPermissionDisplayName(rowDataPermDetailList) {
      if (!rowDataPermDetailList || rowDataPermDetailList.length === 0) {
        return "无条件";
      }

      return rowDataPermDetailList
        .map(detail => {
          // 根据字段编码获取字段名称
          ...
          if (this.entityPermPoints && this.entityPermPoints.rowPermPointList) {
            const permPoint = this.entityPermPoints.rowPermPointList.find(
              point => {
                return point.entityField.fieldCode === detail.fieldCode;
              }
            );
            if (permPoint) {
    ...
    handleRangeTypeChange(point, selectedCode) {
      ...
      // 找到选中的控制方法
      const selectedMethod = tempItem.rowPermControlType.rowPermControlMethods.find(
    ...
    // 工号查询处理
    async handleJobNumberSearch() {
      this.jobNumber = this.$refs.form.getQueryForm()["jobNumber"];
      if (!this.jobNumber) {
        this.resetJobNumberSearch();
        return;
      }
      await this.loadRoleInfo();
      if (this.selectedAsset && this.roleInfo.userId) {
        this.handleAssetSelect(this.selectedAsset, true);
      }
    },
    // 重置工号
    resetJobNumberSearch() {
      console.log("xxxsss");
      this.jobNumber = "";
      this.roleInfo = {};
    ...
    handleOpenPermissionConfig(point, methodCode, type) {
      ...
      if (methodCode === "appoint") {
        ...
        methodCode === "custom" ||
        methodCode === "customize_sql_condition"
      ) {
    ...
    async handleOpenSpecifiedConfig() {
      ...
      try {
        // 获取该字段的可选值
        this.currentFieldOptions = this.allOptions[
          this.currentConfigField.fieldCode
        ];
    ...
    // 打开自定义条件配置
    handleOpenCustomConfig() {
      const tempItem = this.tempPermissionSelections.find(
        item =>
          item.fieldCode === this.currentConfigField &&
          this.currentConfigField.fieldCode
      );
      // 查类型，选操作符
      this.customCondition = tempItem.config
        ? { ...tempItem.config }
        : { operator: "", value: "" };
    ...
    async handleSaveCustomConfig() {
      ...
      try {
        ...
        const validationResult = await rolePermissionApi.checkCustomSqlCondition(
          validationData
        );

        if (!validationResult.code != "0") {
          this.$message.error(
            validationResult.message || "SQL条件校验失败，请检查输入的条件值"
          );
          return;
        }
        ...
        const selectedOperator = availableOperators.find(
          op => op.code === this.customCondition.operator
        );
        const operatorText = selectedOperator
          ? selectedOperator.name
          : this.customCondition.operator;
