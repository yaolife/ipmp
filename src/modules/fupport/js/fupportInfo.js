...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      api
        .getFupportListAPI(fullQueryParams)
    ...
    delFupportInfo(row) {
      ...
      this.$confirm(
        this.$t("cm.delete") + " 支吊架编号为：" + row.hangerNo + " ?",
        this.$t("cm.tips"),
        {
    ...
    onImportFile(e) {
      ...
      if (!allowedExtensions.includes(fileExtension)) {
        this.$message.error(
          "导入文件格式错误，请上传后缀为.xlsx或.xls的Excel文件"
        );
        ...
        .catch(error => {
          ...
          if (error.response) {
            ...
            if (httpCode === 413) errMsg = "上传文件过大，请拆分后重新导入";
              ...
              errMsg = "服务器处理文件异常，请检查文件格式";
            else if (httpCode === 401 || httpCode === 403)
              errMsg = "登录已过期或无导入权限，请重新登录";
