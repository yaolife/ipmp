...
export const Filters = {
  ...
  fieldValue(obj, fieldKey, i18n) {
    ...
    if (fieldKey == "procInstStatus" && i18n) {
      // 流程实例状态显示值转换
      fieldValueDesc = Options.getOptionLabel(
        i18n,
        Options.procInstStatus,
        fieldValue
      );
    }
    if (fieldKey == "procActName" && i18n) {
      fieldValueDesc = Filters.processName(
        obj,
        i18n,
        "procActName",
        "procActEnName"
    ...
    if (fieldKey == "assignTime" && i18n) {
      fieldValueDesc = Filters.splitTime(obj.assignTime);
    }
    if (fieldKey == "completedTime" && i18n) {
      fieldValueDesc = Filters.splitTime(obj.completedTime);
    }
...
export const Options = {
  ...
  getOptionLabel(i18n, options, currentValue) {
    ...
    return selectedOptions.length > 0
      ? i18n.t(selectedOptions[0].labelKey)
      : "";
  }
};
 ...
 * @return {string|string}
 */
export function checkIsXSS(v) {
  let res1 = new RegExp(
    "\\b(document|onload|eval|script|img|svg|onerror|javascript|alert)\\b"
  ).test(v);
  let res2 = new RegExp("<", "g").test(v);
  let res3 = new RegExp(">", "g").test(v);
